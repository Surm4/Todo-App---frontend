        ...
        const todosItems = dataItems.map((todo, index) => 
            <TodoContainer key={ index + "todo-container"}>
                <Todo tabIndex="0" key={ index + "todo"} onClick={ e => { e.preventDefault(); this.mark(todo.id) }}>
                    ...
                    { todo.completed ? <i className="fas fa-check-square" style={{ color: THEME_COMPLETED_FONT_COLOR }}></i> : null }
                </Todo>
                { todo.completed ? <TodoRemover onClick={ e => this.removeItem(todo.id) } tabIndex="0"><i className="fas fa-trash-alt"></i></TodoRemover> : null }
            </TodoContainer>
        );
        return todosItems;
    }
